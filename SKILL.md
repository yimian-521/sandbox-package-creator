...
version: 2.5.0
description: 元Skill——创建沙盒包（v2.5）。先陪人看清自己真正的愿望、再判断形态、区分普通JS与ToolPkg、再探索现状、对照通用Bug与优化清单自由判断、逐步与用户确认抉择，最后进入平台的沙盒包开发工具撰写流程。兼容层级、按类型性能准则、QuickJS陷阱全内置。内置形态区别表、第一次创建必读清单（典型案例+冷门致命错误）。仅在用户明确表达"想做沙盒包/工具/脚本"或主动要求了解沙盒包概念时触发——日常聊天、天气、闲聊不触发。
disable-model-invocation: false
---

# Sandbox Package Creator v2.5
