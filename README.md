...
## 版本 **v2.6.0** (2026-06-26) — 熔断与优化耦合铁律。

- **新增 Bug #45：熔断阈值与返回格式优化不同步**（设计耦合类）——格式越原生→字节越大→正常搜全触发截断。四条修复路径：耦合审查 / 阈值按比例 / 渐进熔断 / 摘要自带字节数
- **新增案例 L**：Search Vault v4.0 真实教训——原生格式下字节数 ×3，阈值未同步上调，十次搜索全变文件保存
- **5.2 搜索类熔断条目扩展**：阈值按「典型结果字节数 × 3」设置，格式每改一次阈值同步审查
- **跳过清单 +1**：不审查输出大小与熔断的耦合
- 行数：~630→~650
...
| [v2.6.0](./SKILL.md) | 722 | 设计耦合族：15条Bug(#45-#59)按8族归类 + 族索引 + 案例L |
