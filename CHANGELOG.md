...
> **当前最新版本：v2.6.0 (2026-06-26)** — sandbox-package-creator ~650行，新增熔断阈值与格式优化耦合铁律（Bug #45 + 案例 L）。v1.0→v2.6 全版本谱系见下方。

## v2.6.0 (2026-06-26)

### sandbox-package-creator — 熔断与优化耦合铁律

**新增：设计耦合类Bug（🟣 JS通用陷阱续，#45）：**

- **熔断阈值与返回格式优化不同步**：格式越原生→字节越大→正常搜全触发截断。两个各自合理的设计（原生格式=更多信息 + 熔断截断=防上下文爆炸），放在一起变成用户搜不到结果。
- **四条修复路径**：①阈值与格式耦合审查 ②阈值按比例而非绝对值 ③渐进熔断 ④返回摘要自带字节数
- **核心判断**：任何涉及输出大小的优化，必须同步审查所有依赖输出大小的下游机制

**新增：案例 L ——「返回格式越优化 → 搜索结果越看不到」**

- Search Vault v4.0 真实教训：原生格式字节数 ×3，auto_save 阈值纹丝不动 → 十次搜索全变文件保存
- **免免原话收录**：「如果但凡用熔断机制，也就是返回的字节太多，那在此之前先思考一下，返回内容优化怎么样……无论是搜索引擎还是其他信息类都很吃紧这种知识点，以至于他们优化忘了改熔断上限。」
- 适用范围：搜索引擎、API聚合、文件处理、日志提取——任何「输出大小」和「截断机制」耦合的工具

**5.2 搜索类性能准则扩展：**

- 熔断阈值条目加 v2.6 警告：每次改返回格式须实测字节数并上调阈值
