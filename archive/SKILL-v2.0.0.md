...
description: 元Skill——创建Operit沙盒包。先陪人看清自己真正的愿望、再判断形态是否真的该做沙盒包、再探索现状、对照通用Bug与优化清单自由判断、逐步与用户确认抉择，最后进入SandboxPackage_DEV撰写流程。兼容层级、按类型性能准则、QuickJS陷阱全内置。
...
| 层级 | 调用方式 | 接口要求 | 适合 |
|------|---------|---------|------|
| 🟢 原生QuickJS | `debug_run_sandbox_script`，stdout/stderr唯一通道 | 返回纯文本即可 | 自己用、一次性脚本 |
| 🟡 Package API | 通过Operit沙盒包系统调用，结构化params/result | `{success, data, error}`统一包装 | 高频工具、团队内用 |
...
- **被替代风险**：Operit 将来原生支持这个功能怎么办？现在做的还有价值吗？
